...
def desample_block(filters: int, size: int):
    ...
    block.add(layers.Conv2D(filters, size, strides=2, padding='same',
                kernel_initializer=initializer, use_bias=False, name='Desample_conv2'))
    block.add(layers.BatchNormalization(name='Desample_bn2'))
    block.add(layers.ReLU(name='Desample_relu2'))
...
def EfficientNet(
  ...
  down_stack = [
      desample_block(320, 3),#8x8 bottleneck (Middle one)
      desample_block(672, 3),
      desample_block(240, 3),
      desample_block(144, 3),
      desample_block(96, 3) # First encode block
  ]
