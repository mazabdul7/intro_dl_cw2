...
if __name__ == "__main__":
    ...
    for cv_iter in range(CVfolds):
        ...
        train_loader = DataLoader(batch_size=batch_size, batch_size_val=batch_size_val, CrossVal=1, CV_iteration=cv_iter, fold=CVfolds)
        val_loader = DataLoader(batch_size=batch_size, batch_size_val=batch_size_val, CrossVal=2, CV_iteration=cv_iter, fold=CVfolds)

        # Train set
        img_ds = train_loader.get_image_ds().repeat()
        masks_ds = train_loader.get_mask_ds().repeat()
        label_ds = train_loader.get_binary_ds().repeat()
        bbox_ds = train_loader.get_bboxes_ds().repeat()
    ...
    with open('model_weights/MTL/CV/CV_seg_bbox.txt', 'w') as file:
