...
def generator_img(img_ds, masks_ds, label_ds):
...
def generator_img_val(img_ds_val, masks_ds_val, label_ds_val):
    ''' Merges together datasets into a unified generator to pass for training '''
...
if __name__ == "__main__":
    ...
    for cv_iter in range(CVfolds):
        ...
        training_loader = DataLoader(batch_size=batch_size, batch_size_val=batch_size_val, CrossVal=1, CV_iteration=cv_iter, fold=CVfolds)
        val_loader = DataLoader(batch_size=batch_size, batch_size_val=batch_size_val, CrossVal=2, CV_iteration=cv_iter, fold=CVfolds)

        # Train set
        img_ds = training_loader.get_image_ds().repeat()
        masks_ds = training_loader.get_mask_ds().repeat()
        label_ds = training_loader.get_binary_ds().repeat()
        bbox_ds = training_loader.get_bboxes_ds().repeat()

        # Validation set
        img_ds_val = val_loader.get_image_ds().repeat()
        masks_ds_val = val_loader.get_mask_ds().repeat()
        label_ds_val = val_loader.get_binary_ds().repeat()
        bbox_ds_val = val_loader.get_bboxes_ds().repeat()
        ...
        model.compile(optimizer=keras.optimizers.Adam(1e-4),
                    ...
                    loss_weights=[1,1], # Scale MAE to BC range
                    metrics=['accuracy'])

        print(f"-------------------- start cross val {cv_iter+1}/{CVfolds} --------------------")

        history = model.fit(generator_img(img_ds, masks_ds, label_ds), 
                            validation_data=generator_img_val(img_ds_val, masks_ds_val, label_ds_val), 
                            epochs=ep, 
    ...
    with open('model_weights/MTL/CV/CV_seg_bin.txt', 'w') as file:
    ...
    ax.set_title('MTL with segmentation and binary')
    plt.legend(loc='lower right')
    plt.ylim([0.75, 1])
    plt.grid()
    fig.savefig('CV_SegBin_plot.png')
