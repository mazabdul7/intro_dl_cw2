...
def generator_img():
    ''' Merges together dataset generators into a unified generator to pass for training '''
    # We use generators like this to abstract shuffling away from the loader and customise yielding
    # directly from the loaders like adding shuffling and augmentation.
...
csv_logger1 = CSVLogger(get_mtl_training_log_path(1), separator=',', append=False)
history = model.fit(generator_img(), validation_data=generator_img_val(), epochs=epochs, steps_per_epoch=num_train//batch_size, validation_steps=num_val//batch_size_val, callbacks=[csv_logger1])
...
csv_logger2 = CSVLogger(get_mtl_training_log_path(2), separator=',', append=False)
history_sec = model.fit(generator_img(), validation_data=generator_img_val(), epochs=epochs, steps_per_epoch=num_train//batch_size, validation_steps=num_val//batch_size_val, callbacks=[csv_logger2])
...
# Save the logs
initial_train_logs = pd.read_csv(get_mtl_training_log_path(1))
fine_tuning_logs = pd.read_csv(get_mtl_training_log_path(2))

merged = initial_train_logs.merge(fine_tuning_logs, on='segnet_out_accuracy')
merged.to_csv(get_mtl_training_log_path(0), index=False)
# Test on test-
